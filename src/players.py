...
class DQN_Player():
    def __init__(self, model_file):
        ...
        self.model.load_state_dict(torch.load(
            model_file, map_location=device)['state_dict'])

    def select_place(self, state, legal_places):
        with torch.no_grad():
            next_actions = torch.tensor(
                [legal_places], device=device, dtype=torch.long)
            q = self.model(state)
            legal_q = q.gather(1, next_actions)
            return legal_places[legal_q.argmax(dim=1).item()]
